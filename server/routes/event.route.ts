...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!userId) {
      res
        .status(400)
        .json({ success: false, error: "No userId provided in body" });
    ...
    try {
      ...
      const event = Event.fromDocument(eventDoc);
      const updatedEvent = await EventService.registerParticipant(
        eventId,
        userId
      );
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!questions || !Array.isArray(questions)) {
      res
        .status(400)
        .json({ success: false, error: "Invalid or missing questions array" });
    }
    ...
    try {
      ...
      res
        .status(200)
        .json({ success: true, message: "Questions added to form" });
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to add questions to form" });
    }
  })
);
